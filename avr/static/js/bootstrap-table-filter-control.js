...
!function ($) {
    ...
    var addOptionToSelectControl = function (selectControl, value, text) {
        ...
        if (existsOptionInSelectControl(selectControl, value)) {
            /* ############### my addition ############# 
            value = value.trim();
            if (value.includes("studentProjectBadge")) {
                var projectName = $('<div />').html(text).text().trim();
                // add project only once, if it doesn't exist yet in the select
                if (selectControl.find("option[value='"+projectName+"']").length == 0) {
    ...
    var createControls = function (that, header) {
        ...
        $.each(that.columns, function (i, column) {
            ...
            if (!column.filterControl) {
                ...
                html.push('<div style="margin: 0px 0px 0px 0px;" class="filterControl">');
                ...
                switch (column.filterControl.toLowerCase()) {
                    case 'input' :
                        html.push(sprintf('<input type="text" placeholder="search..." class="form-control" style="width: 100%; visibility: %s">', isVisible));
                        break;
                    case 'select':
                        /* html.push(sprintf('<select class="%s form-control" style="width: 100%; visibility: %s; padding: 5px;"></select>',
                            column.field, isVisible)) */
                        
                        /* ------------------------------ */
                        /* My edit instead of what above */
                        /* ------------------------------ */

                        html.push(sprintf('<select class="%s custom-select form-control" style="width:100%;min-width: 80px; visibility: %s; padding: 5px;"></select>',
                            column.field, isVisible))
                        break;
                    case 'datepicker':
                    ...
                    if (column.filterControl !== undefined && column.filterControl.toLowerCase() === 'select' && column.searchable) {
                        if (column.filterData === undefined || column.filterData.toLowerCase() === 'column') {
                            ...
                            if (selectControl !== undefined && selectControl.length > 0) {
                                ...
                                /* ################################## */
                                /* my addition instead of what above */
                                /* ################################## */
                                if (column.field in that.filterOptions){
