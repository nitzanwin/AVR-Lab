...
from arvr import app, db, bcrypt, mail
from arvr.forms import (RegistrationForm, LoginForm, EditAccountForm, 
...
from arvr.models import (Student, User, Admin, Project, Supervisor, 
...
@app.route('/Admin/Students/<int:year>/<string:semester>', methods=['POST'])
		...
		if formName == 'addForm':
			...
			if editForm.validate_on_submit():	
				...
				if 'csrf_token' in editForm.errors:
					...
					flash('There was an error, see details below.', 'danger')
				editFormErrorButtonId = "edit"+editForm.proposedProjectId.data
	
	proposedProjects = ProposedProject.query.all()		
	return render_template('/admin/proposedProjects.html', title="Manage Proposed Projects", proposedProjects=proposedProjects, addForm=addForm, editForm=editForm, deleteForm=deleteForm, addFormErrors=addFormErrors, editFormErrorButtonId=editFormErrorButtonId)
	...
	if request.method == "POST":
		if form.validate_on_submit():
			...
			if userToLogIn:
				...
				flash('Login unsuccessful: user not registred.', 'danger')
		else:
